...
Hello, and welcome to the PROJECT_NAME project. We are happy that you are
...
Group. The project is designed to have its ownership transferred to the BOUND_PROJECT_NAME
...
The goal for the PROJECT_NAME project is as follows:
...
3. To create a solid and the de-facto Rust bindings for BOUND_PROJECT_NAME.

Communication
=============

The main ways to communicate with the PROJECT_NAMEing project are as follow:

- [ASWF Slack](https://slack.aswf.io/) under the `#rust` channel.
- [Rust Working Group mailing list](wg-rust@lists.aswf.io)
- [GitHub issues](https://github.com/vfx-rs/PROJECT_NAME/issues)
...
PROJECT_NAME uses
[GitHub's issue tracking system](https://github.com/vfx-rs/PROJECT_NAME/issues)
...
bindings. However, if the bug is with the BOUND_PROJECT_NAME library, then please
[submit an issue with the BOUND_PROJECT_NAME project](https://github.com/AcademySoftwareFoundation/BOUND_PROJECT_NAME/issues).
...
Open a GitHub issue: https://github.com/vfx-rs/PROJECT_NAME/issues
...
The PROJECT_NAME project is licensed under the [BSD 3 clause license](LICENSE).
...
- `build/`: This is automatically generated via cppmm. Inside it is the `ast`
  ...
  for a C++ parser, `PROJECT_NAME-c` folder that contains the C interface for the
  wrapped library, and `PROJECT_NAME-sys` folder that is the unsafe Rust bindings.
- `PROJECT_NAME-rs/`: Contains the safe Rust bindings.

Development and Pull Requests
-----------------------------

Development is done on the main branch. The version branches track major and
minor changes based on the wrapped library. The version of the `PROJECT_NAME-rs`,
`PROJECT_NAME-sys`, `PROJECT_NAME-c`, and the wrapped BOUND_PROJECT_NAME projects should match the
major and minor version. For example, if PROJECT_NAME releases version 1.2.0, then
the other projects should also have that version. If PROJECT_NAME releases version
1.2.3, then the other projects may not have a version that matches due to the
...
All functionality in the `PROJECT_NAME-rs` crate must have associated tests. The
...
version as the BOUND_PROJECT_NAME version it is bound against. The patch version is free to
change as needed when the BOUND_PROJECT_NAME library patches issues, or when the bindings
patch issues.
...
1. Update the release notes in ``CHANGES.md``.
   ...
   Include the log of all changes since the last release, via:
        ...
        git log v2.2.1...v2.3.0 --date=short --pretty=format:"[%s](https://github.com/vfx-rs/PROJECT_NAME/commit/%H) ([%an](@%ae) %ad)"
